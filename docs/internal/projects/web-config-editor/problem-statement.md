...
That edit surface is powerful but brittle. A typo in a biome id, display colour, command toggle, encounter row, or world-data reference can make commands fail later in Discord. The current safety net is `!westmarch check`, which validates the live server config after it is already wired into Avrae.
...
| Maintainer | Keep the browser validator aligned with `!westmarch check` and avoid duplicating server-specific constants in `src/` |
...
- Run a check similar to `!westmarch check` before export or publish.
...
- The validation view mirrors `!westmarch check` severity and coverage where possible, but gives richer browser guidance.
