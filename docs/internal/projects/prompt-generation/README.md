...
> Merge this `world_data_paths` list into `forgotten_realms_2014_paths.gvar.json` under `paths_by_from`, grouped by each path's `from` id. Do not duplicate edges with the same `from`, `to`, and `requirements.transport`.
