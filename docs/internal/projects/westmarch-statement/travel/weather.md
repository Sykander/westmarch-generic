...
```mermaid
flowchart TD
  A[!weather alias] --> B{resolve_config}
  B --> C{travel.commands.weather?}
  C --> D[Resolve area_code]
  D --> E[world_weather.for_location cfg, area, season]
  E --> F[Embed]
```

### Engine: `world_weather.gvar`

| Function | Responsibility |
|----------|----------------|
| `resolve_area_code(config, character, args)` | From arg or journeys + area map |
| `current_season(config)` | Call `world_clock.world_season` if time enabled; else `"default"` |
...
- [ ] **`world_weather.gvar`** — resolve area, pick weather, format embed
