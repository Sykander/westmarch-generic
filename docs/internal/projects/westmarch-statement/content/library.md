...
```mermaid
flowchart TD
  A[!library alias] --> B{resolve_config}
...
| Data | Owner | Notes |
|------|-------|-------|
| `library.gvar` — search, comprehension, censoring | **Engine** | Port per architecture doc; heavy logic stays engine-side |
| Book catalogue (title, author, topics, tags, body) | **Config** gvar | May need **extension gvar** for large corpora ([SS Option C](../solution-statement.md)) |
| Cooldown / cvar key names | **Engine** `bags` | `library_cooldown_code`, `book_comprehension` JSON store |
| Embed footer | **Config** | Server branding |

### Config loader integration

1. `resolve_config()` + `require_command(cfg, "content", "library")`
