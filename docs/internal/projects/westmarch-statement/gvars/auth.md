...
| 5 | **Channel** — `_check_channel(cfg, …)` using `ctx.channel` | `(False, "Run bot commands in #commands, not in RP channels.")` |
| 6 | **Success** | `(True, "Command allowed.")` |

Admin commands skip step 5 when `cfg.channel_policy.admin_any_channel` is true (default **true**).
...
- `_check_channel(subsystem, requires_admin)` — step 5; calls `get_config()` internally

## Channel policy *(optional config)*

If **`channel_policy`** is omitted from the server config gvar, **any guild channel** is allowed (step 5 passes).
