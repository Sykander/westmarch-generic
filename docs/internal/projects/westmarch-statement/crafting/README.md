...
```mermaid
flowchart LR
  alias[Crafting alias] --> loader[resolve_config]
  ...
  roll --> bag[modify_bag on success]
...
| Item catalogue | **`items.gvar`** — search by type | — |
| Spell catalogue | — | **`spells.gvar`** |
| DC / cost tables | **`crafting.gvar`** or config | scroll costs in config |
| Tool prof cvars | **`bags.gvar`** `tool_prof_code`, `tool_exp_code` | same |
